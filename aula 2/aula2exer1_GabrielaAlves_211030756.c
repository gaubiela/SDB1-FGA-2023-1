...
void cadastro_proprietario_veiculo() {
  ...
  while (!cpf_valido) {
    ...
    if (strlen(novo_proprietario.cpf) != 11) {
      printf("CPF invalido, o mesmo deve conter 11 digitos.\n");
    } else {
      cpf_valido = 1;
    }
  }
...
int compara_cpf(const void *a, const void *b) {
  return strcmp(((Proprietario *)a)->cpf, ((Proprietario *)b)->cpf);
}

void imprimir_lista(Proprietario *lista, int tamanho) {
  for (int i = 0; i < tamanho; i++) {
    printf("Nome: %s  CPF: %s\n", lista[i].nome, lista[i].cpf);
    puts("-------------------------------");
    Veiculo busca_veiculo;
    FILE *busca_p = fopen("bda.bin", "r");
    while (fread(&busca_veiculo, sizeof(Veiculo), 1, busca_p)) {
      if (strcmp(lista[i].cpf, busca_veiculo.dono) == 0) {
        printf("marca:%s\ncor:%s\nmodelo:%s\nano:%d\nplaca:%s\n\n",
               busca_veiculo.marca, busca_veiculo.cor, busca_veiculo.modelo,
               busca_veiculo.ano, busca_veiculo.placa);
        puts("");
...
void ordena() {
  ...
  int count = size / sizeof(Proprietario);

  Proprietario proprietarios[count];
  fread(proprietarios, sizeof(Proprietario), count, fp);

  qsort(proprietarios, count, sizeof(Proprietario), compara_cpf);
  imprimir_lista(proprietarios, count);

  fseek(fp, 0, SEEK_SET);
  fwrite(proprietarios, sizeof(Proprietario), count, fp);
...
int main() {
  ...
  for (;;) {
    ...
    printf("(3) - Listar proprietarios ordenados por cpf.\n");
    printf("(4) - Sair.\n\n");
